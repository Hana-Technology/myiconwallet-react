...
function Logo({ className, iconOnly = false, iconClassName, textClassName }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox={iconOnly ? '135 0 120 120' : '0 0 350 120'}
      role="logo"
      aria-label="ICON Foundation logo"
      className={`fill-current inline align-middle ${iconOnly ? '-mt-1' : ''} ${className || ''}`}
      style={iconOnly ? { height: '1.2em' } : { height: '0.68em' }}
      data-testid="logo"
    >
      {/* o icon */}
